...
уникальность source+id, URL, нормализованный текст/работодательская ссылка для
уверенных cross-source дублей. Company+title не достаточно; разные страны/команды
не объединять. Ссылки и пользовательские состояния сохраняются при объединении.
